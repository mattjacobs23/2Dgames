#Credit due to Clear Code
import pygame, sys, random

def draw_floor():
    screen.blit(floor_surface, (floor_x_pos, 640))
    screen.blit(floor_surface, (floor_x_pos + 576, 640))
...
def score_display(game_state):
    ...
    if game_state == 'game_over':
        ...
        high_score_rect = score_surface.get_rect(center=(175, 400))
...
# Game Variables
gravity = 0.25  # has no relation to the real value of gravity
bird_movement = 0
game_active = True
score = 0
high_score = 0
...
pygame.time.set_timer(SPAWNPIPE, 1200) #milli = 0.001 (10^-3 = 1/1000), million = 10^6
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
        if event.type == pygame.KEYDOWN: #check if button pressed (any button)
            if event.key == pygame.K_SPACE and game_active: #check if SPACE is pressed and game still going
                bird_movement = 0
                bird_movement -= 6
        ...
        if event.type == BIRDFLAP:  # 0, 1, 2 -> 0, ...
            if bird_index < 2:
                bird_index += 1
            else:
                bird_index = 0
